...
class ViewAside extends View {
  ...
  #mainHandler() {
    ...
    this.#parent.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.shopping-cart__remove')) {
        const id = e.target.closest('[data-id]').dataset.id;
        this.removeFromCartHandler(id);
      }

      // Handle place button
      if (e.target.closest('.shopping-cart__cta'))
        this.#orderPlacementHandler();
    });
    ...
    this.#parent.addEventListener('keydown', (e) => {
      ...
      // Handle place button
      if (document.activeElement.closest('.shopping-cart__cta'))
        this.#orderPlacementHandler();
    });
  }
  ...
  #orderPlacementHandler() {
    if (this.#itemsContainer.querySelectorAll('[data-id]').length)
      return location.assign(`${ORDER_PAGE}`);

    // If there are no items in the shopping cart
    this.qs('.shopping-cart__cta').classList.add('no-items-animation');
    setTimeout(
      () =>
        this.qs('.shopping-cart__cta').classList.remove('no-items-animation'),
      200
    );
  }
  ...
  renderItems(items) {
    if (items.length === 0)
      return this.#renderEmptyCartError('Your shopping cart is empty');

    // Clear the error
    this.#parent.querySelector('.error')?.remove();
  ...
  removeItem(id) {
    ...
    if (this.#itemsContainer.querySelectorAll('[data-id]').length === 0)
      this.#renderEmptyCartError('Your shopping cart is empty');
  }

  #renderEmptyCartError(message) {
    const markup = `<div class="error">
      ...
      <div class="error__text">${message}</div>
    </div>`;
    this.#itemsContainer.insertAdjacentHTML('beforeend', markup);
  }
}
